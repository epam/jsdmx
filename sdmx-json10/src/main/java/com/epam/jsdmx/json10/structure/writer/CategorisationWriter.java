...
public class CategorisationWriter extends MaintainableWriter<Categorisation> {
    ...
    private void writeArtefactReference(JsonGenerator jsonGenerator, ArtefactReference structure, String fieldName)
            throws IOException {
        if (structure != null) {
            final ArtefactReference resolved = referenceResolver.resolve(structure);
            jsonGenerator.writeStringField(fieldName, referenceAdapter.toAdaptedUrn(resolved));
        }
    }
}
