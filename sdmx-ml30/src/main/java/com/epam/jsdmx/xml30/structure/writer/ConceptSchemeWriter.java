...
public class ConceptSchemeWriter extends XmlWriter<ConceptScheme> {
    ...
    private void writeConcepts(ConceptScheme conceptScheme, XMLStreamWriter writer) throws XMLStreamException {
        ...
        if (CollectionUtils.isNotEmpty(concepts)) {
            for (Concept concept : concepts) {
                writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.CONCEPT);
    ...
    private void writeIsoConcept(XMLStreamWriter writer, Concept concept) throws XMLStreamException {
        ...
        if (isoConceptReference != null) {
            ...
            writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.ISOCONCEPT_REFERENCE);
            ...
            if (agency != null) {
                writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.CONCEPT_AGENCY);
                writer.writeCharacters(agency);
                writer.writeEndElement();
            }

            if (schemeId != null) {
                writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.CONCEPT_SCHEME_ID);
                writer.writeCharacters(schemeId);
                writer.writeEndElement();
            }

            if (conceptId != null) {
                writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.CONCEPT_ID);
