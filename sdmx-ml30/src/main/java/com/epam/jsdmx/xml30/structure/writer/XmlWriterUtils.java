...
public final class XmlWriterUtils {
    ...
    public static void writeConceptIdentity(XMLStreamWriter writer, Component component) throws XMLStreamException {
        ArtefactReference conceptIdentity = component.getConceptIdentity();
        if (conceptIdentity != null) {
            writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.CONCEPT_IDENTITY);
            writeCharacters(conceptIdentity.getUrn(), writer);
            writer.writeEndElement();
        }
    }

    public static void writeConceptRoles(XMLStreamWriter writer, List<ArtefactReference> conceptRoles) throws XMLStreamException {
        if (CollectionUtils.isNotEmpty(conceptRoles)) {
            for (ArtefactReference artefactReference : conceptRoles) {
                if (artefactReference != null) {
                    writer.writeStartElement(XmlConstants.STRUCTURE + XmlConstants.CONCEPT_ROLE);
