...
class ConceptSchemeMapWriterTest extends BaseXmlWriterTest {

    @Test
    void writeConceptSchemeMap() throws IOException, SAXException {
        //given
        XMLUnit.setIgnoreWhitespace(true);

        ConceptSchemeMapWriter conceptSchemeMapWriter = createConceptSchemeMapWriter();

        ByteArrayOutputStream actual = new ByteArrayOutputStream();
        XmlStructureWriter xmlStructureWriter = new XmlStructureWriter(
            actual,
            List.of(conceptSchemeMapWriter),
            headerWriter,
            false
        );
        ...
        ConceptSchemeMapImpl conceptSchemeMap = MaintainableArtifactsTestUtils.buildConceptSchemeMap();
        artefacts.getConceptSchemeMaps().add(conceptSchemeMap);
        xmlStructureWriter.write(artefacts);

        InputStream resourceAsStream = this.getClass().getResourceAsStream(CONCEPT_SCHEME_MAP_XML);
